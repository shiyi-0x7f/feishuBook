...
<el-collapse-item title="getSelectedRecordIdList()" name="getSelectedRecordIdList">
<highlightjs language='javascript' code="//请在一些记录前打上√再运行该段代码
const table = await base.getActiveTable();
const view = await table.getActiveView();
...
"/>
		...
		<el-space direction="horizontal">
			...
			<el-button type="primary" :icon="Pointer" @click="getSelectedRecordIdList" />
			<el-text class="mx-1" type="primary">{{ btn_notice }}</el-text>
		</el-space>
		<br>
		<el-text class="mx-1" type="danger">{{ description }}</el-text>
...
<el-collapse-item title="getChildRecordIdList()" name="getChildRecordIdList">
<highlightjs language='javascript' code="//请在具有子记录的记录行上打上√,否则将返回undefined
const table = await base.getActiveTable();
const view = await table.getActiveView();
const records = await table.getSelectedRecordIdList();//在此之前请选中带有子记录的记录行
const record = records[0];
await view.getChildRecordIdList(record);
"/>
		
		<el-space direction="horizontal">

			<el-button type="primary" :icon="Pointer" @click="getChildRecordIdList" />
			<el-text class="mx-1" type="primary">{{ btn_notice }}</el-text>
		</el-space>
		<br>
		<el-text class="mx-1" type="danger">{{ description }}</el-text>
...
<el-collapse-item title="getFieldWidth()" name="getFieldWidth">
<highlightjs language='javascript' code="const table = await base.getActiveTable();
const view = await table.getActiveView();
const fields = await table.getFieldList();
const field_id = fields[0].id;
const width = await view.getFieldWidth(field_id);
"/>
		<el-space direction="horizontal">
			<el-button type="primary" :icon="Pointer" @click="getFieldWidth" />
			<el-text class="mx-1" type="primary">{{ btn_notice }}</el-text>
		</el-space>
		<br>
		<el-text class="mx-1" type="danger">{{ description }}</el-text>
...
<el-collapse-item title="setFieldWidth()" name="setFieldWidth">
<highlightjs language='javascript' code="//const table = await base.getActiveTable();
const view = await table.getActiveView();
const fields = await table.getFieldList();
const field_id = fields[0].id;
const width = await view.setFieldWidth(field_id);
"/>
<el-input-number
    v-model="width"
    :min="80"
  ...
  /><br />
		<el-space direction="horizontal">
			<el-button type="primary" :icon="Pointer" @click="setFieldWidth(width)" />
			<el-text class="mx-1" type="primary">{{ btn_notice }}</el-text>
		</el-space>
		<br>
		<el-text class="mx-1" type="danger">{{ description }}</el-text>
...
<el-collapse-item title="setRowHeight()" name="setRowHeight">
<highlightjs language='javascript' code="//const table = await base.getActiveTable();
const view = await table.getActiveView();
const fields = await table.getFieldList();
const field_id = fields[0].id;
const width = await view.setRowHeight(field_id);
"/>
<el-input-number
    v-model="height"
    :min="1"
  ...
  /><br />
		<el-space direction="horizontal">
			<el-button type="primary" :icon="Pointer" @click="setRowHeight(height)" />
			<el-text class="mx-1" type="primary">{{ btn_notice }}</el-text>
		</el-space>
		<br>
		<el-text class="mx-1" type="danger">{{ description }}</el-text>
...
export default {
    ...
    methods: {
		...
		async getSelectedRecordIdList(){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
			this.btn_notice = await view.getSelectedRecordIdList();
			
		},
		async getChildRecordIdList(){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
			const records = await view.getSelectedRecordIdList();//在此之前请选中带有子记录的记录行
			const record = records[0];
			this.btn_notice = await view.getChildRecordIdList(record);
			if (this.btn_notice==null){
				this.btn_notice = "请检查是否没有选中带有子记录的记录行"
			}
			this.description = "该函数仅支持下一级子记录的获取，不支持直接获取下下一级的子记录，可以使用递归函数建立层次关系！"
		},
		async getFilterInfo(){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
		...
		async updateSort(){
			...
			sort.desc = !sort.desc;
			await view.updateSort(sort);
			this.description = "更新排序成功";
		},
		async getGroupInfo(){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
			const group_info = await view.getGroupInfo();//在此之前请选中带有子记录的记录行
			if (group_info.length==0){
		...
		async getFieldWidth(){
			...
			const width = await view.getFieldWidth(field_id);
			const field_name = await fields[0].getName();
			this.btn_notice = `[${field_name}] 宽度为：${width}`
		},
		async setFieldWidth(w){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
			const fields = await table.getFieldList();
			const field_id = fields[0].id;
			await view.setFieldWidth(field_id,w);
			const width = await view.getFieldWidth(field_id);
			const field_name = await fields[0].getName();
			this.btn_notice = `[${field_name}] 宽度为：${width}`
		},
		async setRowHeight(h){
			const table = await base.getActiveTable();
			const view = await table.getActiveView();
			this.btn_notice = await view.setRowHeight(h);
			 //= `[${field_name}] 宽度为：${width}`
		},
