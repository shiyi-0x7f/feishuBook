...
<highlightjs language='javascript' code="//请在多选字段中查看变化
const table = await base.getActiveTable();
const type = FieldType.MultiSelect;
const filed_list = await table.getFieldListByType(type);
const mt_field = filed_list[0];//获取返回的第一个单选字段
await mt_field.addOption(cv,1)
"/>
...
await mt_field.addOptions(options);
"/>
...
const filed_list = await table.getFieldListByType(type);
const mt_field = filed_list[0];//获取返回的第一个单选字段
const ops = await mt_field.getOptions();
"/>
...
await mt_field.deleteOption(op_id);
"/>
		<el-space direction="horizontal">
...
const ops = await mt_field.getOptions();
const op_id = ops[0].id;
await mt_field.setOption(op_id,{
	name:cell_value
});
"/>
...
const filed_list = await table.getFieldListByType(type);
const mt_field = filed_list[0];//获取返回的第一个单选字段
const ops_type = await mt_field.getOptionsType();
"/>
...
await mt_field.setOptionsType(new_type);
"/>
...
export default {
    ...
    methods: {
		...
		async addOption(cv){
            ...
            const filed_list = await table.getFieldListByType(type);
            const mt_field = filed_list[0];//获取返回的第一个单选字段
            this.btn_notice = await mt_field.addOption(cv,1)
        },
		async addOptions(cv){
		    const table = await base.getActiveTable();
		    const type = FieldType.MultiSelect;
		    ...
		    this.btn_notice = await mt_field.addOptions(options);
		...
		async getOptions(){
		    ...
		    const filed_list = await table.getFieldListByType(type);
		    const mt_field = filed_list[0];//获取返回的第一个单选字段
		    this.btn_notice = await mt_field.getOptions();
		},
		async deleteOption(){
		    const table = await base.getActiveTable();
		    const type = FieldType.MultiSelect;
		...
		async getOptionsType(){
		    ...
		    const filed_list = await table.getFieldListByType(type);
		    const mt_field = filed_list[0];//获取返回的第一个单选字段
		    this.btn_notice = await mt_field.getOptionsType();
        ...
        async setOptionsType(){
            ...
            const filed_list = await table.getFieldListByType(type);
            const mt_field = filed_list[0];//获取返回的第一个单选字段
            const ops_type = await mt_field.setOptionsType();
			let new_type = ops_type?0:1;//如果是0变为1,如果是1变为0
			this.btn_notice = await mt_field.setOptionsType(new_type);
